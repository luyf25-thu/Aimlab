...
public:
    ...
    // 设置网格间距和大小缩放（靶子散布范围适配分辨率）
    void setSpacingScale(float scale)
    {
        spacingScale = std::max(1.0f, scale);
        if (modeRef) modeRef->setSpacingScale(spacingScale);
    }
    ...
    void update(float deltaTime, const sf::Vector2u& areaSize)
    {
        ...
        while (activeCount < desiredCount)
        {
            ...
            if (modeRef)
            {
                ...
                target->init(info.position, info.radius * radiusScale, info.lifeTime, info.requiredHits, info.baseColor);
